      ...
      stopTime = window.setInterval(function () {
        if (runing) {
          ...
          if ( Lotterynumber == inUser) {
            // 指定中奖人
            nametxt.css('background-image','url(image/2.jpg)');
            phonetxt.html('指定中奖人');
            $('.luck-user-list').prepend("<li><div class='portrait' style='background-image:url(image/2.jpg)'></div><div class='luckuserName'>aaa</div></li>");
            $('.modality-list ul').append("<li><div class='luck-img' style='background-image:url(image/2.jpg)'></div><p>aaa</p></li>");
            inUser = 9999;
          }else{
            //打印中奖者名单
